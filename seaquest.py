...
def train_agent(env_name, episodes=1000, render=False):
    ...
    for e in range(1, episodes+1):
        ...
        if e % 100 == 0:
            agent.save(f"{save_dir}/seaquest_dqn_ep{e}.pt")
            plot_training_results(e, scores, average_scores, epsilon_values)
    
    # Save final model
    agent.save(f"{save_dir}/seaquest_dqn_final.pt")
...
def plot_training_results(episodes, scores, avg_scores, epsilons):
    ...
    plt.tight_layout()
    plt.savefig(f"{save_dir}/seaquest_training_results_ep{episodes}.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    model_path = f"{save_dir}/seaquest_dqn_final.pt"
